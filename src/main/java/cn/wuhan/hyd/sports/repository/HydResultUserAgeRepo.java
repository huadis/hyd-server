 ...
 */
@Repository
public interface HydResultUserAgeRepo extends JpaRepository<HydResultUserAge, Long> {
    /**
     * 统计用户渠道
     */
    @Query(value = "SELECT " +
            ...
            "over50Num AS '50岁以上' " +
            "FROM hyd_result_user_age WHERE statisticalYear = ?1 " +
            "ORDER BY createdTime DESC LIMIT 1", nativeQuery = true)
    Map<String, Object> countStadiumUserAgeStat(String year);

    @Modifying
    @Query(value = "DELETE FROM hyd_result_user_age WHERE batchNo != ?1 AND statisticalYear = ?2", nativeQuery = true)
