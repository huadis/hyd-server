...
public interface HydResultOrderYktProjectStatRepo extends JpaRepository<HydResultOrderYktProjectStat, Long> {
    ...
    @Query(value = "SELECT * FROM hyd_result_order_ykt_project_stat WHERE statisticalYear = ?1 ORDER BY num DESC", nativeQuery = true)
    List<HydResultOrderYktProjectStat> listProject(String year);

    @Modifying
    @Query(value = "DELETE FROM hyd_result_order_ykt_project_stat WHERE statisticalYear = ?1", nativeQuery = true)
