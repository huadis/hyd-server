...
public interface HydExcelPublicEventsHistoryRepo extends JpaRepository<HydExcelPublicEventsHistory, Long> {
     ...
     * 总赛事场次
     */
    @Query(value = "SELECT eventYear as statisticalYear, COUNT(id) AS total FROM hyd_excel_public_events_history WHERE eventYear is not null and eventYear != '' GROUP BY eventYear", nativeQuery = true)
    List<Map<String, Object>> countAll();

    /**
     * 总参与人数
     */
    @Query(value = "SELECT eventYear as statisticalYear, SUM(participantCount) AS participantCount FROM hyd_excel_public_events_history WHERE eventYear is not null and eventYear != '' GROUP BY eventYear", nativeQuery = true)
    List<Map<String, Object>> totalParticipantCount();

    /**
     * 国际赛事场次
     */
    @Query(value = "SELECT eventYear as statisticalYear, COUNT(id) AS internationalCount FROM hyd_excel_public_events_history  WHERE eventYear is not null and eventYear != '' and eventLevel = '国际级' GROUP BY eventYear ", nativeQuery = true)
    List<Map<String, Object>> internationalEventCount();

    /**
     * 国家级赛事场次
     */
    @Query(value = "SELECT eventYear as statisticalYear, COUNT(id) AS nationalCount FROM hyd_excel_public_events_history WHERE eventYear is not null and eventYear != '' and  eventLevel = '国家级' GROUP BY eventYear ", nativeQuery = true)
    List<Map<String, Object>> nationalEventCount();

    /**
     * 省级
     */
    @Query(value = "SELECT eventYear as statisticalYear, COUNT(id) AS provinceCount FROM hyd_excel_public_events_history WHERE eventYear is not null and eventYear != '' and eventLevel = '省级' GROUP BY eventYear ", nativeQuery = true)
    List<Map<String, Object>> provinceCount();

    /**
     * 市级
     */
    @Query(value = "SELECT eventYear as statisticalYear, COUNT(id) AS cityCount FROM hyd_excel_public_events_history WHERE eventYear is not null and eventYear != '' and eventLevel = '市级' GROUP BY eventYear ", nativeQuery = true)
    ...
    @Query(value = "SELECT " +
            ...
            "    ) AS rn " +
            "  FROM hyd_excel_public_events_history " +
            "  WHERE " +
    ...
    @Query(value = "SELECT eventYear AS statisticalYear , " +
            ...
            "    COUNT(*) AS count " +
            "FROM " +
            "    hyd_excel_public_events_history WHERE eventYear is not null and eventYear != '' " +
            "GROUP BY eventYear, " +
            "    CASE " +
            "        WHEN participantCount < 100 THEN '<100人' " +
            "        WHEN participantCount BETWEEN 100 AND 300 THEN '100-300人' " +
    ...
    @Query(value = "SELECT " +
            ...
            "    eventDate " +
            "FROM " +
            "    hyd_excel_public_events_history " +
            "WHERE " +
