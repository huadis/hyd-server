...
public interface HydResultStadiumDistrictRepo extends JpaRepository<HydResultStadiumDistrict, Long> {
     ...
     */
    @Query(value = "SELECT * FROM hyd_result_stadium_district WHERE YEAR(createdTime) = ?1 and districtName is not null and districtName != '' order by stadiumNum desc", nativeQuery = true)
