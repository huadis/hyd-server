...
public interface HydOriginStadiumItemHistoryRepo extends JpaRepository<HydOriginStadiumItemHistory, String> {
    ...
    @Query(value = "select sportName, count(*) as num from hyd_origin_stadium_item_history WHERE sportName IS NOT NULL AND sportName != '' AND createdTime >= DATE_FORMAT(CURDATE(), '%Y-01-01 00:00:00') AND createdTime < DATE_FORMAT(DATE_ADD(CURDATE(), INTERVAL 1 YEAR), '%Y-01-01 00:00:00') GROUP BY sportName order by num desc limit 10", nativeQuery = true)
