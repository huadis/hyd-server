...
public interface HydResultFacilityDistrictRepo extends JpaRepository<HydResultFacilityDistrict, Long> {
     ...
     * @return 设施全貌
     */
    @Query(value = "SELECT * FROM hyd_result_facility_district WHERE statisticalYear = ?1 AND facilityTypeName = ?2 ORDER BY CAST(facilityNum AS UNSIGNED) DESC", nativeQuery = true)
