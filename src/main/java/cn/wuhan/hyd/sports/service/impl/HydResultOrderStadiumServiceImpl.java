...
public class HydResultOrderStadiumServiceImpl implements IHydResultOrderStadiumService {
    ...
    public int batchSave(List<HydResultOrderStadium> orderStadiums) {
        ...
        List<HydResultOrderStadiumHistory> historyList = convertToHistoryList(orderStadiums, batchNo);
        try {
            // 4. 清空查询表：日志记录操作意图，便于问题追溯
            logger.info("【批量保存】开始清空HydResultOrderStadium表，批次号：{}", batchNo);
            orderStadiumRepo.deleteAll();
            ...
            if (querySaveCount != historySaveCount || querySaveCount != orderStadiums.size()) {
                throw new RuntimeException(
                        String.format("【批量保存】数据保存数量不一致，批次号：%s，原数据量：%d，查询表保存量：%d，历史表保存量：%d",
                                batchNo, orderStadiums.size(), querySaveCount, historySaveCount)
                );
            }
     ...
     */
    private List<HydResultOrderStadiumHistory> convertToHistoryList(
            List<HydResultOrderStadium> sourceList,
            String batchNo) {
        try {
            return sourceList.stream()
                    .map(source -> {
                        HydResultOrderStadiumHistory history = new HydResultOrderStadiumHistory();
                        try {
