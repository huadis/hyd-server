...
public class HydResultStadiumMapServiceImpl implements IHydResultStadiumMapService {
    ...
    public int batchSave(List<HydResultStadiumMap> stadiumMaps) {
        ...
        List<HydResultStadiumMapHistory> historyList = convertToHistoryList(stadiumMaps, batchNo);
        try {
            // 4. 清空查询表：日志记录操作意图，便于问题追溯
            logger.info("【批量保存】开始清空HydResultStadiumMap表，批次号：{}", batchNo);
            stadiumMapRepo.deleteAll();
     ...
     */
    private List<HydResultStadiumMapHistory> convertToHistoryList(
            List<HydResultStadiumMap> sourceList,
            String batchNo) {
        try {
            return sourceList.stream()
                    .map(source -> {
                        HydResultStadiumMapHistory history = new HydResultStadiumMapHistory();
                        try {
