 ...
 */
@Service
public class HydOriginTrainingActivityServiceImpl implements IHydOriginTrainingActivityService {
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public int batchSave(List<HydOriginTrainingActivity> trainingActivities) {
        // 验证参数
        ...
        if (trainingActivities.size() > 1000) {
            throw new IllegalArgumentException("单次导入最大支持1000条数据");
        }
        String batchNo = UUIDUtil.getBatchNo();
        // 数据转换：Stream流+异常封装, 提前转换失败直接终止
        List<HydOriginTrainingActivityHistory> historyList = convertToHistoryList(trainingActivities, batchNo);
        try {
     ...
     */
    private List<HydOriginTrainingActivityHistory> convertToHistoryList(
            List<HydOriginTrainingActivity> sourceList,
            String batchNo) {
        try {
            return sourceList.stream()
                    .map(source -> {
                        HydOriginTrainingActivityHistory history = new HydOriginTrainingActivityHistory();
                        try {
