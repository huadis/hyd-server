...
public class HydResultStadiumSportCouponServiceImpl implements IHydResultStadiumSportCouponService {
    ...
    public int batchSave(List<HydResultStadiumSportCoupon> stadiumSportCoupons) {
        ...
        List<HydResultStadiumSportCouponHistory> historyList = convertToHistoryList(stadiumSportCoupons, batchNo);
        try {
            // 4. 清空查询表：日志记录操作意图，便于问题追溯
            logger.info("【批量保存】开始清空HydResultStadiumSportCoupon表，批次号：{}", batchNo);
            stadiumSportCouponRepo.deleteAll();
            ...
            if (querySaveCount != historySaveCount || querySaveCount != stadiumSportCoupons.size()) {
                throw new RuntimeException(
                        String.format("【批量保存】数据保存数量不一致，批次号：%s，原数据量：%d，查询表保存量：%d，历史表保存量：%d",
                                batchNo, stadiumSportCoupons.size(), querySaveCount, historySaveCount)
                );
            }
     ...
     */
    private List<HydResultStadiumSportCouponHistory> convertToHistoryList(
            List<HydResultStadiumSportCoupon> sourceList,
            String batchNo) {
        try {
            return sourceList.stream()
                    .map(source -> {
                        HydResultStadiumSportCouponHistory history = new HydResultStadiumSportCouponHistory();
                        try {
